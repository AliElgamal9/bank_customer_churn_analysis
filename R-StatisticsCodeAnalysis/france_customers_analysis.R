...
france_customers <- subset(bank_data, Geography == "France")

# mark the account status as factor, the factor to group by 
france_customers$AccountStatus <- as.factor(france_bank_data$AccountStatus)
...
leveneTest(Satisfaction.Score ~ AccountStatus, data = france_customers)
t.test(Satisfaction.Score ~ AccountStatus, data = france_customers, var.equal = TRUE)
# no significant difference (p-value = 0.5548), difference CI = [-0.07, 0.1]

# test for estimated salary ------------------------------------------
leveneTest(EstimatedSalary ~ AccountStatus, data = france_customers)
t.test(EstimatedSalary ~ AccountStatus, data = france_customers, var.equal = TRUE)
cohen.d(EstimatedSalary ~ AccountStatus, data = france_customers)
...
leveneTest(MembershipYears ~ AccountStatus, data = france_customers)
t.test(MembershipYears ~ AccountStatus, data = france_customers, var.equal = TRUE)
# no significant difference (p-value =  0.997), diff CI = [-.2, .2]

# test for credit score ----------------------------------------------
leveneTest(CreditScore ~ AccountStatus, data = france_customers)
t.test(CreditScore ~ AccountStatus, data = france_customers, var.equal = TRUE)
cohen.d(CreditScore ~ AccountStatus, data = france_customers)
...
# test for age -------------------------------------------------------
leveneTest(Age ~ AccountStatus, data = france_customers)
t.test(Age ~ AccountStatus, data = france_customers, var.equal = FALSE)
cohen.d(Age ~ AccountStatus, data = france_customers)
# significant difference (p-value < 2.2e-16), diff CI = [7, 8.6] closed higher
# effect size is 0.78 (medium)

# test for gender ------------------------------------------------------
gender_table <- table(france_customers$Gender, france_customers$AccountStatus)
# display table
print(gender_table)
